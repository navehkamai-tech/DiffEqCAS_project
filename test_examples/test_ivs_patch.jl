...
J_inv = zeros(Num, 0, 0)
var_map = Dict()
new_ivs = [@variables(r)[1], @variables(theta)[1]] # dummy
old_u = Symbolics.unwrap.([t, x, y])
new_u = Symbolics.unwrap.(new_ivs)

for dv in sys_dvs
    rewritten = custom_rewrite(Symbolics.unwrap(dv), var_map, J_inv, new_ivs, old_u, new_u, dv_funcs)
    println(dv, " => ", rewritten)
end
